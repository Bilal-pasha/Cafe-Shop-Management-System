...
namespace CafeShopManagementSystem
{
    public partial class CashierOrderForm : UserControl
    {
        ...
        public void displayTotalPrice()
        {
            ...
            if (connect.State == ConnectionState.Closed)
            {
                try
                {
                    ...
                    string selectData = "SELECT SUM(prod_price * qty) FROM orders WHERE customer_id = @custId AND delete_order IS NULL";
            ...
            else
            {
                if (connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        connect.Open();
                        float getPrice = 0;
                        string selectOrder = "SELECT * FROM products WHERE prod_id = @prodID AND prod_status = 'Available' AND date_delete IS NULL";
        ...
        public void IDGenerator()
        {
            ...
            {
                connect.Open();
                string selectID = "SELECT ISNULL(MAX(customer_id), 0) FROM customers";

                using (SqlCommand cmd = new SqlCommand(selectID, connect))
                {
                    object result = cmd.ExecuteScalar();

                    // If no previous customer exists, start from 1
